...
struct gps_pos_t {
  float lat[2]; // current[0] and previous[1]
  float lon[2];
...
int  parse_nmea( char* nmea, char* ans, struct gps_pos_t* gps_pos ) {
  ...
  if ( strcmp("GPGGA", msg) == 0 ) {
    ...
    if ( strcmp( "0", items[6]) == 0 ) {
      ...
      snprintf( ans, MAX_TEXT_LEN, "lat=%f %s lon=%f %s\n", lat, items[3], lon, items[5] );
...
int main( int argc, char *argv[] ) {
  ...
  /*
  static struct gps_msgbuf mb = {2, 0, "Initialised"};
  int msqid = msgget( gps_queue_id, 0666 | IPC_CREAT );
  res = msgsnd( msqid, &mb, sizeof(mb), IPC_NOWAIT );
  if ( res == -1 ) {
    LOG( "Error msgsnd, errno %i (%s)", errno, strerror(errno) );
    return 1;
  }
  ++msg_num;
  */
